...
getName(); // gets called immediately and sets the userName to display.
function getName(){
  fetch(`/name`) // no need for options parameter for GET.
  .then(response => {
    console.log("before json; ",response);
    return response.text(); // ret text wrapped in promise (when sending text from server, we need to receive a text (not json))
  })
  .then(function(data){
