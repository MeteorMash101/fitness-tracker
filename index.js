...
// Let's build a server pipeline! [PIPE STAGE]
...
app.get('/*',express.static('public')); // now default route
...
app.get('/auth/logout', isAuthenticated, (request, response, next) => {
  request.logout();
  response.redirect('/public/splash.html');
});
...
// used for *DEBUGGING PURPOSES* - dumps 2 whole databases
...
app.get('/allProfiles', async function(request, response, next) {
  console.log("Server recieved a get /allProfiles request at", request.url);
  let results = await dbo.get_all_profiles();
  response.send(results);
});

// This is where the sever recieves and responds to a name GET request
app.get('/name', isAuthenticated, async function(request, response, next) {
  console.log("Server recieved a GET request at /name", request.url);
  // Call handler for name
  console.log("IN /NAME: ",request.user.userData.name);
  response.send(request.user.userData.name);
});
...
app.post('/store', isAuthenticated, async function(request, response, next) {
  ...
  let user_id = request.user.userData.userid; // safely extract userid for each query (do not set global!)
  // console.log("TYPE OF USER ID HERE: !!! ", typeof(user_id));
  // console.log("activity ID HERE; ",request.body);  
  // console.log("user ID HERE; ",user_id);
  try {
    await dbo.post_activity(user_id, activity);
  } catch {
    console.log("error! could not post activity...");
  }
...
app.get('/reminder', isAuthenticated, async function(request, response, next) {
  ...
  let user_id = request.user.userData.userid;
  // Get Most Recent Past Planned Activity and Delete All Past Planned Activities
  let result = await dbo.get_most_recent_planned_activity_in_range(user_id, 0, currTime);
  await dbo.delete_past_activities_in_range(user_id, 0, currTime);
  if (result != null) {
...
app.get('/week', isAuthenticated, async function(request, response, next) {
  console.log("Server recieved a post request at", request.url);
  console.log("I'm in the week get request!")
  ...
  if (activity === undefined) {
    let result = await dbo.get_most_recent_entry(user_id)
    try {
  ...
  let max = date
  let result = await dbo.get_similar_activities_in_range(user_id, activity, min, max)
...
// dbo.testDB().catch( // TEMPORARY (USE THIS TO RESET DB'S)
//   function (error) {
...
function gotProfile(accessToken, refreshToken, profile, done) {
    ...
    // console.log("profile userID HERE: ",userid,"is logged in");
    // console.log("profile name HERE: ",name,"is logged in");    
    dbo.check_if_user_exists(userid) // returns [] if not in there...
    .then((userExists) => {
      if (userExists == 0) {
        console.log("USER DOESN'T EXIST - CREATING NEW");
        dbo.add_entry_to_profile(userid, name) // adds to profile db
        .then(() => console.log("successfully updated profile table."))
...
passport.deserializeUser((userid, done) => {
    ...
    dbo.get_userData_from_id(userid) // make this toString?
    .then ((data) => {
      console.log("DATA FROM DE-SER: ",data, typeof(data));
      let userData = {userData: data}; // data from user's db row goes here
      // cud of just passed in userID here to userData
      console.log("userData FROM DE-SER", userData, typeof(userData))
      done(null, userData);
    })
    .catch(err => console.log(err));
});