 ...
 * @params user_id, {Activity} activity 
 * @returns activity components in a list format
 */
function ActivityToList(user_id, activity) {
    if (activity == null || user_id == null) {
        throw new ActivityFormatException(activity)
    }

    return [user_id, activity.activity, activity.date, activity.scalar]
}